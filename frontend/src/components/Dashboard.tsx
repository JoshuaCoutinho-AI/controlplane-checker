  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full p-5 bg-panel border-r border-hairline/60">
      ...
      <div className="flex items-center justify-between bg-ink border border-hairline/60 rounded-xl px-3.5 py-2 mb-6">
        <span className="text-[10px] uppercase font-mono tracking-wider text-muted">Gateway Status</span>
        <div className="flex items-center gap-2">
          <span className={`h-2.5 w-2.5 rounded-full ${connected ? 'bg-pass' : 'bg-block'}`} />
      ...
      <div className="bg-ink/50 border border-hairline/40 rounded-xl p-3.5 space-y-2.5 text-xs">
        <p className="text-muted font-light leading-relaxed">{activePolicy.description}</p>
        <div className="pt-2.5 border-t border-hairline/35 space-y-2 font-mono text-[10px]">
          <div className="flex justify-between">
            ...
            <span className="text-paper font-bold">{activePolicy.latency_budget_ms}ms</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted">BLOCK LOG THRESHOLD</span>
            <span className="text-paper font-bold">&lt;{activePolicy.score_block_below} score</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted">WARN LOG THRESHOLD</span>
            <span className="text-paper font-bold">&lt;{activePolicy.score_log_below} score</span>
          </div>
        </div>
      </div>
        ...
        <header className="hidden md:flex justify-between items-center px-6 py-4 border-b border-hairline/30 bg-panel/10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-muted uppercase tracking-wider">
              Monitoring Mode: <span className="text-paper font-semibold">{activePolicy.name}</span>
            </span>
          </div>
          <ThemeToggle />
        </header>
          ...
          {isEmpty ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 max-w-md mx-auto">
              ...
              <p className="text-muted text-sm leading-relaxed mb-6">
                Submit a prompt to run safety checks live. Select a policy profile in the sidebar to configure latency budgets and routing thresholds.
              </p>
              
              <div className="w-full bg-panel/30 border border-hairline/60 rounded-xl p-4 text-left font-mono text-xs space-y-3">
                <p className="text-brass-soft font-semibold uppercase tracking-wider text-[10px]">Things to try:</p>
                <ul className="space-y-2 text-muted">
                  <li className="flex items-start gap-1.5">
                    <span className="text-brass-soft">•</span>
                    <span>Leave response blank to generate a real LLM reply</span>
                  </li>
                  <li className="flex items-start gap-1.5">
                    <span className="text-brass-soft">•</span>
                    <span>Paste custom email or phone response to trigger PII block</span>
                  </li>
                  <li className="flex items-start gap-1.5">
                    <span className="text-brass-soft">•</span>
                    <span>Verify how routing changes under different profiles</span>
                  </li>
                </ul>
              </div>
            </div>
