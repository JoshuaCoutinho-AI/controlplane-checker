  ...
  return (
    <div className="flex flex-col gap-1.5 min-w-[80px]">
      <div className="flex items-baseline justify-between">
        ...
        <span className={`font-mono text-xs font-semibold tabular-nums ${GAUGE_COLOR(score)}`}>{score}</span>
      </div>
...
export default function ResponseCard({ record }: { record: ScoredResponse }) {
  ...
  const [showOverrideInput, setShowOverrideInput] = useState<'allow' | 'block' | null>(null)
  ...
  async function handleOverride(status: 'override_allow' | 'override_block' | 'none', reason?: string) {
  ...
  async function handleFeedback(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/score/${id}/feedback`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ feedback_text: feedbackInput }),
      })
      if (res.ok) {
        setFeedbackInput('')
      }
  ...
  return (
    <div className="space-y-4 py-4 border-b border-hairline/40 w-full max-w-3xl mx-auto px-4">
      ...
      <div className="flex flex-col items-start gap-1.5 w-full">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2 font-mono text-[9px] text-muted">
            ...
            {generated && (
              <>
                <span>•</span>
                <span className="text-brass-soft">generated ({llm_provider})</span>
              </>
            )}
          </div>
          <div className="flex items-center gap-2">
            {override_status !== 'none' && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono uppercase tracking-wider font-semibold ${
                override_status === 'override_allow' ? 'bg-pass-soft text-pass border border-pass/30' : 'bg-block-soft text-block border border-block/30'
              }`}>
                Human Override: {override_status === 'override_allow' ? 'Allowed' : 'Blocked'}
              </span>
            )}
        ...
        <div className={`px-4 py-3 rounded-2xl max-w-[85%] text-sm relative select-text w-full ${
          severity === 'block' 
          ...
          {severity === 'block' ? (
            <div className="space-y-2">
              ...
              <p className="text-muted/80 text-xs italic">
                The original model response was withheld because of compliance violations.
              </p>
        ...
        <div className="flex flex-wrap items-center gap-3 mt-1 w-full pl-1">
          <button
            ...
            className={`font-mono text-[10px] uppercase tracking-wider flex items-center gap-1 ${
              showAnalysis ? 'text-brass font-bold' : 'text-muted hover:text-paper'
            }`}
          >
          ...
          {override_status === 'none' ? (
            <>
              {severity === 'block' && (
                <button
                  type="button"
                  onClick={() => setShowOverrideInput(showOverrideInput === 'allow' ? null : 'allow')}
              ...
              {severity === 'pass' && (
                <button
                  type="button"
                  onClick={() => setShowOverrideInput(showOverrideInput === 'block' ? null : 'block')}
            ...
            >
              <RefreshCw size={10} className={submittingOverride ? 'animate-spin' : ''} />
          ...
          <div className="w-full bg-panel/30 border border-hairline/30 rounded-xl p-4 mt-2 space-y-4 animate-fade-in">
            ...
            <div className="space-y-1.5 pt-2 border-t border-hairline/20">
              <div className="flex items-start gap-1 text-[11px]">
                <span className="font-mono text-muted uppercase tracking-wider">Gateway Decision:</span>
                <span className="font-mono text-paper font-semibold uppercase">{severity}</span>
              </div>
              <div className="flex items-start gap-1 text-[11px]">
                <span className="font-mono text-muted uppercase tracking-wider">Reason:</span>
            ...
            {showOverrideInput && (
              <div className="bg-ink/60 border border-hairline p-3 rounded-lg space-y-2">
                <p className="font-mono text-[10px] uppercase text-brass-soft">
                  Provide a Reason for Human {showOverrideInput === 'allow' ? 'Approval' : 'Blocking'}
                </p>
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => handleOverride(showOverrideInput === 'allow' ? 'override_allow' : 'override_block', overrideReason)}
                    disabled={submittingOverride}
                  ...
                  <button
                    type="button"
                    onClick={() => { setShowOverrideInput(null); setOverrideReason(''); }}
            ...
            {/* Feedback Loop Form */}
            <form onSubmit={handleFeedback} className="flex gap-2 pt-2 border-t border-hairline/20">
              <input
                type="text"
                value={feedbackInput}
                onChange={(e) => setFeedbackInput(e.target.value)}
                placeholder="Log feedback notes (e.g., model hallucinated, great phrasing)..."
